...
The distance between them is $\frac{2}{\|\mathbf{w}\|}$ so, in order to maximize the distance $\|\mathbf{w}\|$ has to be minimized. To ensure that no data points lie within the margin, the following constraint must be satisfied:
$$
